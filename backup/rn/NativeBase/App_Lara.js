...
export default class App extends Component {
  ...
  async getAlertWeb() {
    return fetch("http://192.168.100.83/laravel55/public/api/tasks")
  ...
  addTask = () => {
    fetch('http://192.168.100.83/laravel55/public/api/task', {
      method: 'POST',
      headers: {
...
completeTask = (id) => { 
  fetch(`http://192.168.100.83/laravel55/public/api/task/${id}/complete`)
  .done();    
};

deleteTask = (id) => { 
  fetch(`http://192.168.100.83/laravel55/public/api/task/${id}/delete`)
  .done();    
};

editTask = (id) => { 
    fetch(`http://192.168.100.83/laravel55/public/api/task/${id}/edit`)
    .done();    
  };
              ...
              <ListItem style = {{ marginTop:5, }}>
                ...
                <Right>
                    <TouchableOpacity onPress={ () => {this.editTask(item.id)} } hitSlop={{top: 20, bottom: 20, left: 50, right: 50}}>
                        {/* <Icon name="ios-checkmark" style={{fontSize: 30, color: '#000000'}} /> */}
