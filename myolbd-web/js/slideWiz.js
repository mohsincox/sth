...
$.fn.slideWiz = function (e) {
    ...
    const slideWiz = (element, e) => {
        ...
        const getDetail = (position) => {
            ...
            if (isString(e.file[position].titles)) {
                slideDetailBox.append(
                    $('<h4>').attr("class", "detail-title").html(e.file[position].titles)
                );
            }
                ...
                if (isString(e.file[position].button.url)) {
                    ...
                    slideDetailBox.append(
                        $('<a>').attr({'href': e.file[position].button.url, 'target': '_blanck'}).html(
                            $('<button>').attr({
                                class: "detail-button btn1 " + (isString(e.file[position].button.class) ?
            ...
            if (isObject(e.file[position].button)) {
                ...
                if (isString(e.file[position].button.urls)) {

                    slideDetailBox.append(
                        $('<a>').attr({'href': e.file[position].button.urls, 'target': '_blanck'}).html(
                            $('<button>').attr({
                                class: "detail-button btn2 " + (isString(e.file[position].button.class) ?
                                    e.file[position].button.class : "")
                            }).html((
                                isString(e.file[position].button.texts) ?
                                    e.file[position].button.texts : "Button"
                            ))
                        )
                    );
